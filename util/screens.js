...
export let lastAuthUser = null;

/** Boot screen */
export async function boot() {
	clear();

	await type("Welcome to LUMON industries(TM) terminal", {
		initialWait: 3000
	});
...
export async function login() {
	...
	// Auto-launch whoisit game instead of main terminal
	const { parse } = await import("./io.js");
	return parse("whoisit");
}
...
export async function authenticate({
	...
	clear();
	const user = await prompt(usernameLabel);
	let password;

	if (requirePassword) {
		password = await prompt(passwordLabel, true);
	}
